...
> = ({ statutValidation, data, loading, paginate, habilitation }) => {
  ...
  const [principalEmail, setPrincipalEmail] = useState<any>("");
  ...
  const handleToKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter" || e.key === "Tab" || e.key === " ") {
      ...
      if (email && validateEmail(email) && !toEmails.includes(email)) {
        setToEmails((prevEmails) => [...prevEmails, email]);
        setToInput("");
        ToastSuccess.fire({
      ...
      } else if (toEmails.includes(email)) {
        ToastError.fire({
  ...
  const handleSendEmail = async () => {
    ...
    for (const email of toEmails) {
      if (!validateEmail(email)) {
        ToastError.fire({
          text: `L'adresse email "À" "${email}" est invalide.`,
        });
        return;
      }
    }
    ...
    for (const email of ccEmails) {
      if (!validateEmail(email)) {
        ToastError.fire({
          text: `L'adresse email CC "${email}" est invalide.`,
        });
        return;
      }
    }
          ...
          >
            <div className="fixed inset-0 z-[999] overflow-y-auto bg-[black]/60">
              <div className="flex min-h-screen items-start justify-center px-4">
                <Dialog.Panel className="panel my-8 w-full max-w-4xl overflow-hidden rounded-lg border-0 px-4 py-1">
                  ...
                  <div className="mb-4">
                    ...
                    <div className="form-input mt-1 flex w-full flex-wrap items-center rounded-md border border-gray-300 bg-white p-2 shadow-sm focus-within:ring-2 focus-within:ring-orange-500 sm:text-sm">
                      {toEmails.map((email, index) => (
                        <div
                          key={index}
                          className="mb-1 mr-2 flex items-center rounded bg-blue-100 px-2 py-1 text-sm text-blue-800"
                        >
                          {email}
                          <button
                            type="button"
